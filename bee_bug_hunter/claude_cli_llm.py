...
Session-based prompt caching, reserved-manager + root-fork topology: the
Investigation Manager (MANAGER_ROLE_NAME, config.py) gets its own reserved
`claude -p` session per flow investigation -- minted directly on its own first
call, never forked from anything, and never forked FROM by anything either.
Every worker instead forks its own session off a separate, generic, content-free
"root" session (shared framing only: flow name, containers -- no tool schemas)
on its first call, then reuses that fork on every later call, sending only the
messages appended since the previous call.
...
callable this turn. An earlier version of this file had workers fork directly
off the manager's own live session instead of a generic root, on the theory
that richer shared context (the manager's actual investigation-so-far, not just
flow/container framing) would help. Verified live against a real flow
(2026-07-30): it backfired -- the manager's own system prompt carries all six
handoff tool schemas, and that leaked into every worker's forked history, so
workers started hallucinating handoff-tool calls (e.g. the API Flow Runner
worker attempting to call "docker_log_capturer", a tool it was never given),
wasting a retry each time and once cascading into a fatal CLI error
("No deferred tool marker found"). Root going back to content-free is what
keeps each role's tool schema actually isolated to that role.
...
def _load_persisted_sessions() -> dict[str, dict]:
    """{flow_name: {"root": session_id, "roles": {role: session_id}}}, persisted
    to disk so sessions survive process restarts (a crash, or a fresh --once
    invocation) instead of living only in ClaudeCLIChatModel._instances, an
    in-memory dict that resets every run. "root" is the generic, content-free
    session every worker forks from (_ensure_root_session); the manager's own
    reserved session lives under roles[MANAGER_ROLE_NAME] like any other role,
    but -- unlike a worker's -- is never forked from root or anything else."""
...
def _ensure_root_session(flow_key: str, flow_name: str, containers: str, model: str) -> str:
    """Creates (once) the shared root session every WORKER forks its own
    session from (see ClaudeCLIChatModel.for_role()) -- generic, role-agnostic
    framing only, deliberately with no tool schemas, so no worker's forked
    history ever contains another role's tool descriptions. The manager does
    NOT fork from this (or from anything) -- see for_role(); an earlier design
    had workers fork directly from the manager's own live session instead of
    this content-free root, verified live to cause exactly the cross-role tool
    confusion this separation exists to prevent (see module docstring). A raw
    standalone `claude -p` call rather than a ChatModel instance, since it isn't
    itself a worker/manager role and has no BeeAI-side conversation of its own."""
...
class ClaudeCLIChatModel(ChatModel):
    ...
    manager (MANAGER_ROLE_NAME) gets its own reserved session, minted directly
    and never forked from anything; every worker forks its own persistent CLI
    session off a separate, content-free root session on first use (see
    for_role()), then reuses that fork on every later turn -- see the module
    docstring for the full rationale, including why workers no longer fork from
    ...
    def for_role(
        ...
        if cache_key not in cls._instances:
            ...
            # The manager gets its own reserved session -- never forked from
            # anything (fork_from=None lets __init__/`_invoke_cli` mint a fresh
            # standalone session on its own first real call, same as any
            # direct-construction-outside-for_role() case). Every worker forks
            # from the shared, content-free root instead: forking from the
            # manager's own live session was tried and verified live to leak the
            # manager's tool schema into worker history, causing hallucinated
            # handoff-tool calls -- see module docstring and _ensure_root_session().
            fork_from = None if role_key == MANAGER_ROLE_NAME else _ensure_root_session(
                flow_key, flow_name or flow_key, containers, model,
            )
            cls._instances[cache_key] = cls(
                model=model, flow_key=flow_key, role_key=role_key,
                session_id=existing_role_session, fork_from=fork_from, **kwargs,
            )
    ...
    async def _create(self, input: ChatModelInput, run) -> ChatModelOutput:
        ...
        if self._session_id is None:
            # First call for this role was either the manager minting its own
            # standalone session or a worker forking from root; adopt and persist
    ...
    def _invoke_cli(self, system_prompt: str, user_prompt: str) -> tuple[str, str]:
        ...
        order: (1) first-ever call for this worker role -> fork off the shared
        root session (--resume <root_id> --fork-session), letting the CLI assign
        the new session_id, which _create() then adopts and persists; (2)
        resuming a session already known (from this process or a persisted prior
        one) (--resume <id>); (3) no fork_from and no session_id -- reached both
        by the manager's own first-ever call (for_role() deliberately passes
        fork_from=None for MANAGER_ROLE_NAME) and by direct construction outside
        for_role() (e.g. tests) -- starts a plain standalone session
